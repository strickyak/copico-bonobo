    ...
    while (true) {
...
#if 0
        volatile uint early, addr;
        {
            uint spin = 0;
            do {
                // Read twice, and check twice, to avoid glitches.
                early = volatile_sio_hw->gpio_in;
                addr = volatile_sio_hw->gpio_hi_in & 0xFFFF;
                ++spin;
            } while ((early & (1<<G_E)) != 0);
        ...
        if (writing) {
          // CPU WRITING -- we RX
            ...
            byte late = (byte)volatile_sio_hw->gpio_in;  // late grab of data
            ram[addr] = late;
          ...
          if (addr >= 0xFF00) {
...
#if CENTIPEDE_CTS
          } else if ((early & (1u << CENTIPEDE_CTS)) == 0) {
#else
              ...
              STALL_WHILE(G_E , not CENTIPEDE_INVERT_EQ, 't');
              gpio_put(G_SLENB, 1);

              gpio_set_dir_in_masked(0xFF);
#endif
          } else {
              STALL_WHILE(G_E , not CENTIPEDE_INVERT_EQ, 'u');
          }
          // END READING
        } // end if (writing) else
    } // while true
...
void FloppyWriteCommand(uint a, byte d) {
    ...
    floppy_status = ((d & 0xF0) == 0x80) || ((d & 0xF0) == 0xA0) ? 0x02: 0x00; // YAK
