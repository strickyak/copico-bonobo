...
struct DoSsd1306 {
  ...
  static void Ssd1306_Init(uint base) {
    base &= 0xFF;
    Readers[base + DISPLAY_X_PORT] = [](uint addr, byte data) {
      return Display_x;
    };
    Readers[base + DISPLAY_Y_PORT] = [](uint addr, byte data) {
      return Display_y;
    };
    Writers[base + DISPLAY_X_PORT] = [](uint addr, byte data) {
      Display_x = data;
      printf(" display x=%d\n", data);
    };
    Writers[base + DISPLAY_Y_PORT] = [](uint addr, byte data) {
      Display_y = data;
      printf(" display y=%d\n", data);
    };
    Writers[base + DISPLAY_COMMAND_PORT] = [](uint addr, byte data) {
