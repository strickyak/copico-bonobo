...
FORCE_INLINE bool volatile_gpio_get(uint pin) {
...
CrossCoreFIFO<uint, 8*1024> cross;

#define  multicore_fifo_push_blocking(X)  cross.push(X)
#define  multicore_fifo_pop_blocking()  cross_pop()

FORCE_INLINE uint cross_pop() {
    uint z;
    bool ok;
    do {
     ok = cross.pop(z);
...
uint histo[100][26];
void ShowHisto(uint addr, byte d) {
    multicore_fifo_push_blocking(FIFO_DUMP);
}
...
#define STALL_WHILE(PIN,HL,CH) {                                 \
    while (volatile_gpio_get(PIN) == HL) {}                      \
}
...
#define STALL_WHILE(PIN,HL,CH) {                                 \
    ...
    while (volatile_gpio_get(PIN)==HL) {++spin;}                 \
...
void foreground() {
    ...
    while (true) {
        ...
        uint signals, abus;
        do {
            signals = volatile_sio_hw->gpio_in;
            abus = volatile_sio_hw->gpio_hi_in & 0xFFFF;
        } while ((signals & (1<<G_E)) != 0);  // negated E.
#else
        STALL_WHILE(G_E, CENTIPEDE_INVERT_EQ, 'v');
        volatile uint early = volatile_sio_hw->gpio_in;
        uint addr = sio_hw->gpio_hi_in & 0xFFFF;

#define signals early
#define abus addr
#endif
    ...
    if ((floppy_latch&0x80)!=0 && floppy_ptr >= floppy_limit) {
            ...
            multicore_fifo_push_blocking(FIFO_NMI);
              ...
              //SAY('W');
              STALL_WHILE(G_Q , not CENTIPEDE_INVERT_EQ, 'x');
    ...
    *floppy_ptr++ = dbus;
    if ((floppy_latch&0x80)!=0 && floppy_ptr >= floppy_limit) {
        ...
        multicore_fifo_push_blocking(FIFO_NMI);
              ...
              STALL_WHILE(G_E , not CENTIPEDE_INVERT_EQ, 'y');
            } // end read or write
        } // end if special
    } // end while true
} // end foreground()
