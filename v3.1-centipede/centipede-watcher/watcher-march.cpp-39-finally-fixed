...
class NegativeSignal {
    ...
    FORCE_INLINE static bool Get(void) {
        return gpio_get(GPIN);
    }
};
...
template <class T>
class DoCoverRam32K {
    FORCE_INLINE static bool Predicate(uint a) {
        return a < 0x8000;
    }
    FORCE_INLINE static void Write(uint a, byte d) {
        // ram[a] = d // should already be done.
    }
    FORCE_INLINE static byte Read(uint a) {
        return ram[a];
    }
};
...
class DoCoco3Mmu {
    ...
    static void WriteFF91(uint a, byte d) {
        MmuTask = 1u & d;
    }
};
